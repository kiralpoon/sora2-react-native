...
docs/ui-preview.html    # Interactive mock showcasing every planned screen for screenshots
```

## UI Preview & Screenshots

- Open `docs/ui-preview.html` directly in a browser or serve the `docs/` directory (for example `python -m http.server`).
- Use the navigation chips at the top of the preview to switch between **Model Explorer**, **Prompt Builder**, **Parameter Tuner**,
  **Submission Review**, **Job History**, **Result Viewer**, and **Settings**.
- Each screen mirrors the planned React Native layout, making it easy to capture high-fidelity screenshots before native assets
  are implemented.
- When documenting progress, capture one screenshot per screen so stakeholders can see the entire feature set at a glance.
...
3. Submit the request—the API client in `src/api/sora2/client.ts` issues authenticated POST requests to `/v1/videos` (with the `OpenAI-Beta: video-generation=2024-12-17` header) and polls `/v1/videos/{id}` for status updates while honoring watermark controls.
4. Browse the catalog with `GET /v1/models?type=video`; the client normalizes capabilities, aspect ratios, and defaults before persisting them locally.
5. When a job completes, the job tracker aggregates events so notifications and history screens stay in sync.
